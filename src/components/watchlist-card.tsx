      ...
      <div className="flex items-start justify-between gap-3">
        <button type="button" onClick={() => open(item.player)} className="flex min-w-0 items-center gap-3 text-left">
          ...
          <div className="min-w-0">
            <p className="truncate text-lg font-black text-[var(--ink)]">{item.player.name}</p>
            <p className="text-sm font-bold text-[var(--muted)]">{item.player.team} / {item.player.position} / {formatPrice(item.player.price)}</p>
          </div>
        </button>
        <span className={`rounded-lg px-3 py-1 text-xs font-black ${tone}`}>{item.status}</span>
      </div>
      <p className="mt-4 text-sm font-semibold leading-6 text-[var(--muted)]">{item.reason}</p>
      <div className="mt-4 rounded-xl bg-[var(--surface-2)] p-3">
        <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--muted)]">Trigger</p>
        <p className="mt-1 text-sm font-black text-[var(--ink)]">{item.trigger}</p>
      </div>
      <div className="mt-4"><FixturePill fixture={item.player.fixture} difficulty={item.player.fixture_difficulty} /></div>
      <div className="mt-4 flex gap-2">
        <button type="button" className="flex-1 rounded-xl bg-[var(--accent)] px-3 py-2 text-xs font-black text-white">Update group</button>
        <button type="button" className="flex-1 rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] px-3 py-2 text-xs font-black text-[var(--ink)]">Remove</button>
      </div>
    </article>
  );
}
