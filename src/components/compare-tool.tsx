...
function TrendDot({ direction }: { direction: "up" | "down" }) {
  return <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${direction === "up" ? "bg-[var(--success)]" : "bg-[var(--warning)]"}`} aria-hidden />;
}

function BreakdownRow({ label, value, trend, zebra }: { label: string; value: ReactNode; trend?: "up" | "down" | null; zebra: boolean }) {
  return (
    <div className={`grid min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-3 rounded-lg px-3 py-2 text-xs ${zebra ? "bg-[var(--surface-3)]" : ""}`}>
      <span className="min-w-0 break-words font-bold capitalize text-[var(--muted)]">{label}</span>
      <span className="flex min-w-0 items-start justify-end gap-1.5 break-words text-right font-black tabular-nums text-[var(--ink)]">
...
function KeyValueGrid({ data }: { data: unknown }) {
  if (!isRecord(data) || !Object.keys(data).length) return <p className="text-xs font-semibold text-[var(--muted)]">No data supplied.</p>;
  ...
  return (
    <div className="grid gap-0.5">
      ...
      {weighted.length ? (
        <>
          <p className="mt-3 border-t border-[var(--border-soft)] pt-3 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--muted)]">Recent weighted</p>
  ...
  return (
    <div
      className={`relative flex min-h-[46px] min-w-0 items-center justify-center rounded-xl border px-2 py-2.5 text-center text-xs font-black tabular-nums sm:min-h-0 sm:px-3 sm:py-3 sm:text-sm ${
        winning ? "border-[var(--success-border)] bg-[var(--success-soft)] text-[var(--success)]" : "border-[var(--border-soft)] bg-[var(--surface)] text-[var(--ink)]"
      }`}
    >
      {winning ? <span className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-[var(--success)] text-[10px] text-white">✓</span> : null}
...
function MetricBattleRow({
  ...
  return (
    <div className="border-b border-[var(--border-soft)] py-4 last:border-0 sm:grid sm:grid-cols-[minmax(160px,1fr)_110px_110px] sm:items-center sm:gap-4 sm:py-3">
      <div className="min-w-0">
        <p className="text-sm font-black text-[var(--ink)]">{label}</p>
        {note ? <p className="mt-0.5 text-[11px] font-semibold leading-4 text-[var(--muted)]">{note}</p> : null}
      </div>

      <div className="mt-3 grid min-w-0 grid-cols-2 gap-2 sm:contents">
        <div className="min-w-0">
          <p className="mb-1.5 truncate text-[10px] font-black uppercase tracking-[0.08em] text-[var(--muted)] sm:hidden">{playerAName}</p>
          <MetricValue value={a} winning={aWins} />
        </div>
        <div className="min-w-0">
          <p className="mb-1.5 truncate text-[10px] font-black uppercase tracking-[0.08em] text-[var(--muted)] sm:hidden">{playerBName}</p>
...
function PlayerHero({ player, label, winner }: { player: Player; label: string; winner: boolean }) {
  ...
  return (
    <article className={`relative overflow-hidden rounded-[24px] border bg-[var(--surface)] p-4 shadow-[0_18px_50px_rgba(15,23,60,0.08)] sm:p-6 ${winner ? "border-[var(--success)] ring-2 ring-[var(--success)]/12" : "border-[var(--border)]"}`}>
      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[var(--accent)] via-[var(--info)] to-[var(--success)]" />
      <div className="flex items-start justify-between gap-3">
        <span className="rounded-full bg-[var(--accent-soft)] px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--accent)]">{label}</span>
        {winner ? <span className="rounded-full bg-[var(--success-soft)] px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[var(--success)]">Model pick</span> : null}
      </div>
      ...
      <button type="button" onClick={() => open(player)} className="mt-5 flex w-full items-center gap-4 text-left">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-2xl font-black tracking-tight text-[var(--ink)] sm:text-3xl">{player.name}</h2>
          <p className="mt-1 text-sm font-bold text-[var(--muted)]">{player.team} · {player.position} · {formatPrice(player.price)}</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
        <div className="rounded-xl bg-[var(--surface-3)] p-3">
          <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[var(--muted)]">Next GW</p>
          <p className="mt-1 text-xl font-black text-[var(--ink)]">{formatMetric(player.projected)}</p>
        </div>
        <div className="rounded-xl bg-[var(--surface-3)] p-3">
          <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[var(--muted)]">3 GW</p>
          <p className="mt-1 text-xl font-black text-[var(--ink)]">{formatMetric(player.three_gw_projected)}</p>
        </div>
        <div className="col-span-2 rounded-xl bg-[var(--surface-3)] p-3 sm:col-span-1">
          <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[var(--muted)]">Owned</p>
          <p className="mt-1 text-xl font-black text-[var(--ink)]">{formatMetric(player.ownership, "%")}</p>
        </div>
      </div>
    </article>
  );
...
function VerdictPanel({ comparison }: { comparison: PlayerComparison }) {
  ...
  return (
    <aside className="relative overflow-hidden rounded-[24px] border border-[#12182A] bg-[#080B16] p-5 text-white shadow-[0_26px_70px_rgba(0,0,0,0.24)] sm:p-6">
      <div className="absolute -right-14 -top-16 h-44 w-44 rounded-full bg-[var(--accent)]/20 blur-2xl" />
      ...
      <div className="relative">
        ...
        {verdict.why.length ? (
          <div className="mt-5 rounded-2xl border border-white/10 bg-[var(--surface)]/[0.06] p-4">
...
function RoleMapCard({ label, breakdown }: { label: string; breakdown: PlayerBreakdown | null }) {
  ...
  if (breakdown.mapsStatus === "loading") {
    return <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm font-semibold text-[var(--muted)]">Loading {label}&apos;s role map…</div>;
  }
  if (breakdown.mapsStatus === "unavailable" || !breakdown.heatmap) {
    return <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm font-semibold text-[var(--muted)]">Role map unavailable for {label}.</div>;
  }
  return (
    ...
    <div className="space-y-4">
      {breakdown.decision ? (
        <>
          ...
          {(["trend_analysis", "fixture_analysis", "role_analysis", "market_analysis"] as const).map((section) => (
            <div key={section} className="rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)] p-4">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--accent)]">{SECTION_LABELS[section]}</p>
              <div className="mt-3"><KeyValueGrid data={breakdown.decision?.[section]} /></div>
            </div>
          ))}
          <details className="rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)] p-4">
            <summary className="cursor-pointer text-xs font-black uppercase tracking-[0.12em] text-[var(--accent)]">Model explanation and evidence</summary>
      ...
      {breakdown.projection ? (
        <div className="rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)] p-4">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--accent)]">Projection provenance</p>
...
function PlayerBreakdownPanel({
  ...
  const action = (
    <button
      ...
      className="rounded-xl border border-[var(--accent-border)] bg-[var(--surface)] px-4 py-2 text-xs font-black text-[var(--accent)] shadow-sm disabled:cursor-wait disabled:opacity-60"
    >
  ...
  return (
    <>
      <details className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[0_18px_50px_rgba(15,23,60,0.06)] lg:hidden">
        <summary className="cursor-pointer list-none">
          <div className="flex items-center gap-3">
            <PlayerVisual player={player} size="sm" />
            <div className="min-w-0 flex-1">
              <p className="truncate font-black text-[var(--ink)]">{player.name}</p>
              <p className="text-xs font-semibold text-[var(--muted)]">Deep model analysis</p>
            </div>
        ...
        <div className="mt-4 border-t border-[var(--border-soft)] pt-4">
          {action}
          {status === "error" ? <p className="mt-3 text-xs font-bold text-[var(--danger)]">{error}</p> : null}
          {status === "ready" && breakdown ? <div className="mt-4"><BreakdownContent breakdown={breakdown} /></div> : null}
        </div>
      </details>

      <section className="hidden rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_18px_50px_rgba(15,23,60,0.06)] lg:block">
        <div className="flex items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <PlayerVisual player={player} size="sm" />
            <div className="min-w-0">
              <p className="truncate text-lg font-black text-[var(--ink)]">{player.name}</p>
              <p className="text-xs font-semibold text-[var(--muted)]">Decision, role, market and projection evidence</p>
            </div>
          </div>
          {action}
        </div>
...
export function CompareTool({ initialComparison, players }: { initialComparison: PlayerComparison; players: Player[] }) {
  ...
  return (
    <div className="min-w-0 space-y-6">
      <section className="min-w-0 rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[0_18px_55px_rgba(15,23,60,0.07)] sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--accent)]">Head-to-head lab</p>
            <h2 className="mt-2 text-2xl font-black tracking-tight text-[var(--ink)] sm:text-3xl">Compare any two Premier League players</h2>
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[var(--muted)]">Use the live projection, fixture, market and role models to decide who deserves the squad place.</p>
          </div>
          <div className="flex flex-wrap gap-2 text-[11px] font-black">
            <span className="rounded-full bg-[var(--success-soft)] px-3 py-1.5 text-[var(--success)]">Live projections</span>
            <span className="rounded-full bg-[var(--accent-soft)] px-3 py-1.5 text-[var(--accent)]">Full-league directory</span>
            <span className="rounded-full bg-[var(--info-soft)] px-3 py-1.5 text-[var(--info)]">Role maps</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-end lg:grid-cols-[1fr_auto_1fr_auto]">
          ...
          <button
            type="button"
            onClick={swapPlayers}
            className="mx-auto grid h-11 w-11 place-items-center rounded-full border border-[var(--border)] bg-[var(--surface)] text-lg font-black text-[var(--accent)] shadow-sm transition hover:-rotate-180 sm:mb-0"
          ...
          <button
            ...
            className="rounded-xl bg-[var(--accent)] px-5 py-3 text-sm font-black text-white shadow-[0_12px_28px_rgba(108,29,255,0.22)] disabled:cursor-not-allowed disabled:opacity-40 sm:col-span-3 lg:col-span-1"
          >
            {fetchState === "loading" ? "Comparing…" : selectionDirty ? "Run comparison" : "Comparison current"}
          </button>
        </div>

        {selectionDirty ? <p className="mt-3 text-xs font-bold text-[var(--accent)]">Selection changed. Run the comparison to update the verdict and metrics.</p> : null}
        {directoryError ? <p className="mt-3 text-xs font-semibold text-[var(--warning)]">Full player directory could not load; currently known players remain available.</p> : null}
        {fetchState === "error" ? <p className="mt-3 text-xs font-bold text-[var(--danger)]">{fetchError}</p> : null}
      </section>
      ...
      <section className="rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[0_18px_55px_rgba(15,23,60,0.07)] sm:p-6">
        <div className="flex flex-col gap-3 border-b border-[var(--border-soft)] pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--accent)]">Metric battle</p>
            <h2 className="mt-1 text-2xl font-black text-[var(--ink)]">Where each player wins</h2>
          </div>
          <div className="hidden grid-cols-[1fr_110px_110px] gap-4 text-center text-[11px] font-black uppercase tracking-[0.08em] text-[var(--muted)] sm:grid">
            <span />
        ...
        <div className="mt-4 grid min-w-0 grid-cols-2 gap-2 sm:hidden">
          <div className="min-w-0 rounded-xl bg-[var(--accent-soft)] px-3 py-2">
            <p className="text-[9px] font-black uppercase tracking-[0.1em] text-[var(--accent)]">Player A</p>
            <p className="truncate text-xs font-black text-[var(--ink)]">{playerA.name}</p>
          </div>
          <div className="min-w-0 rounded-xl bg-[var(--success-soft)] px-3 py-2">
            <p className="text-[9px] font-black uppercase tracking-[0.1em] text-[var(--success)]">Player B</p>
            <p className="truncate text-xs font-black text-[var(--ink)]">{playerB.name}</p>
          </div>
        </div>
      ...
      {comparison.verdict.why_this_could_be_wrong.length ? (
        <section className="rounded-[22px] border border-[var(--warning-border)] bg-[var(--warning-soft)] p-5">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[var(--warning)]">What could change the verdict?</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {comparison.verdict.why_this_could_be_wrong.map((item) => (
              <p key={item} className="rounded-xl bg-[var(--surface)]/80 p-3 text-sm font-semibold leading-6 text-[var(--warning)]">{item}</p>
            ))}
          </div>
        </section>
      ) : null}
      ...
      <section>
        <div className="mb-4">
          <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--accent)]">Advanced player intelligence</p>
          <h2 className="mt-1 text-2xl font-black text-[var(--ink)]">Load the evidence behind the comparison</h2>
          <p className="mt-2 text-sm font-semibold text-[var(--muted)]">Trend, fixture, role, market and projection provenance remain optional so the main decision stays fast to scan.</p>
        </div>
      ...
      {[breakdownA, breakdownB].some((breakdown) => breakdown && breakdown.mapsStatus !== "idle") ? (
        <section className="space-y-4">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--accent)]">On-pitch role comparison</p>
            <h2 className="mt-1 text-2xl font-black text-[var(--ink)]">Where they actually operate</h2>
          </div>
      ...
      {selectionDirty ? (
        <div className="fixed inset-x-3 bottom-20 z-30 sm:hidden">
          <button
            ...
            className="w-full rounded-2xl bg-[var(--accent)] px-5 py-4 text-sm font-black text-white shadow-[0_18px_50px_rgba(57,20,130,0.35)] disabled:opacity-50"
          >
